...
Java_com_jarvisquest_app_ai_LlamaNative_nativeInit(JNIEnv *env, jobject, jstring modelPath) {
    ...
    cp.n_threads = std::max(1, std::min(8, (int)(hw == 0 ? 4 : hw)));
    cp.n_threads_batch = cp.n_threads;
    llama_context *ctx = llama_init_from_model(model, cp);
    if (!ctx) { LOGE("Failed to create llama context"); llama_model_free(model); llama_backend_free(); return 0; }
    auto *h = new LlamaHandle{model, ctx, llama_model_get_vocab(model)};
    LOGI("Qwen model loaded; threads=%d", cp.n_threads);
...
Java_com_jarvisquest_app_ai_LlamaNative_nativeGenerate(JNIEnv *env, jobject, jlong handle, jstring prompt, jint maxTokens) {
    ...
    const int limit = std::max(1, std::min((int)maxTokens, 96));
    std::string result;
    ...
    for (int i = 0; i < limit; ++i) {
        const llama_token tok = llama_sampler_sample(sampler, h->ctx, -1);
